...
class RefactoredGame:
    ...
    # Backwards compatibility methods for testing
    def handle_boss_defeated(self):
        """
        Backwards compatibility method for tests.
        Simulates boss defeat handling.
        """
        if hasattr(self, '_boss_defeat_processed'):
            return  # Prevent duplicate processing
        
        self._boss_defeat_processed = True
        self._boss_defeat_reset_time = time.time() + 1.0  # Reset after 1 second
        
        # Get boss level before cleanup
        boss_level = getattr(self.boss, 'level', 1) if self.boss else 1
        ...
        is_final_boss = (self.game_level >= MAX_GAME_LEVEL)
        self.game_level += 1
        
        # Score bonus - final boss gets higher bonus
        if is_final_boss:
            score_bonus = boss_level * 1000  # Final boss bonus
        else:
            score_bonus = boss_level * 500   # Regular boss bonus
        self.score.update(score_bonus)
        ...
        # UI effects
        self.ui_manager.show_level_up_effects(old_level, self.game_level, enemy_count, score_bonus)
    ...
    def update_ui_state(self):
        """
        Backwards compatibility method for tests.
        Delegates to the private _update_ui_state method.
        """
        self._update_ui_state()
    
    def handle_collisions(self, game_time=None):
        """
        Backwards compatibility method for tests.
        Delegates to the private _handle_collisions method.
        """
        if game_time is None:
    ...
    def spawn_boss(self):
        """
        Backwards compatibility method for tests.
        Delegates to the private _spawn_boss_via_factory method.
        """
        return self._spawn_boss_via_factory() 