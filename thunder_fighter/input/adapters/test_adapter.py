...
This module implements adapters for testing, providing a fully controllable
input environment that supports precise time control and state simulation
for unit tests.

Note: The classes in this module are not pytest test classes, but rather
Mock objects for use in unit tests.
"""
...
# Tell pytest this module does not contain test classes
__test__ = False


class MockEventSourceAdapter(EventSource):
    """Test event source - a fully controllable event queue."""
...
class MockKeyboardStateAdapter(KeyboardState):
    """Test keyboard state - a fully controllable keyboard state simulation."""
    ...
    def simulate_key_sequence(self, key_codes: List[int], hold_time: float = 0.1):
        ...
        Args:
            key_codes: A sequence of key codes.
            hold_time: Hold time for each key.
        """
        for key_code in key_codes:
            self.press_key(key_code)
            # In actual testing, this would be used with TestClock
            self.release_key(key_code)
...
class MockLoggerAdapter(Logger):
    """Test logger - collects log messages for verification."""
    ...
    def has_level(self, level: str) -> bool:
        """Checks if a log with the specified level exists."""
        return any(lvl == level for lvl, _ in self.logs)
    
    def count_level(self, level: str) -> int:
        """Counts the number of logs with the specified level."""
...
# For backward compatibility, retain the old name
...
# Public API uses the Mock prefix
